...
import { Search, Package, Clock, CheckCircle2, Truck, ShieldCheck, AlertCircle, CreditCard, Image as ImageIcon } from "lucide-react";
                        ...
                        <Card key={booking._id} className={`mb-10 border-none shadow-2xl rounded-3xl overflow-hidden transition-all duration-500 hover:shadow-blue-100 ${bIdx === 0 ? 'ring-2 ring-blue-500' : ''}`}>
                            <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-8 text-white flex justify-between items-center">
                                ...
                                <div className="text-right">
                                    <p className="text-blue-100 text-[10px] font-bold uppercase tracking-[0.2em] mb-2 opacity-80 decoration-amber-400">Smart Estimate</p>
                                    <h3 className="text-xl font-black text-amber-300">
                                        {booking.estimatedReadyDate ? new Date(booking.estimatedReadyDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : "Calculating..."}
                                    </h3>
                                </div>
                            </div>
                            ...
                            <CardContent className="p-8 md:p-10">
                                ...
                                {/* Visual Proof (New) */}
                                {(booking.pickupPhoto || booking.deliveryPhoto) && (
                                    <div className="mb-8 p-6 bg-slate-50 rounded-3xl border border-slate-100 animate-in slide-in-from-right duration-700">
                                        <div className="text-[10px] font-black uppercase text-slate-400 mb-4 tracking-widest flex items-center">
                                            <ImageIcon size={14} className="mr-2" /> Visual Handling History
                                        </div>
                                        <div className="flex gap-4">
                                            {booking.pickupPhoto && (
                                                <div className="flex-1 group">
                                                    <div className="aspect-square rounded-2xl overflow-hidden mb-2 border-2 border-white shadow-sm ring-1 ring-slate-100 relative">
                                                        <img src={booking.pickupPhoto} alt="Pickup Proof" className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                                                        <div className="absolute inset-0 bg-blue-600/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                                    </div>
                                                    <p className="text-[8px] font-black uppercase text-slate-500 text-center">In safe Hands (Pickup)</p>
                                                </div>
                                            )}
                                            {booking.deliveryPhoto && (
                                                <div className="flex-1 group">
                                                    <div className="aspect-square rounded-2xl overflow-hidden mb-2 border-2 border-white shadow-sm ring-1 ring-slate-100 relative">
                                                        <img src={booking.deliveryPhoto} alt="Delivery Proof" className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                                                        <div className="absolute inset-0 bg-green-600/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                                    </div>
                                                    <p className="text-[8px] font-black uppercase text-slate-500 text-center">Reached Safely (Delivered)</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
