...
export default function StaffTerminal() {
    ...
    const loadDrivers = async () => {
        try {
            const res = await fetch(`${API_URL}/api/drivers/list`, {
    ...
    const assignDriver = async (bookingId, driverDbId, type) => {
        try {
            ...
            if (res.ok) {
                alert(`Driver assigned for ${type}!`);
                loadBookings();
            }
        } catch (err) {
                            ...
                            <CardContent className="p-6 space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center pr-1">
                                         ...
                                         <button 
                                            onClick={() => {
                                                const dId = prompt(`Available Drivers:\n${drivers.map(d => `${d.driverId}: ${d.name}`).join('\n')}\n\nEnter Driver ID to assign:`);
                                                if (dId) {
                                                    const driver = drivers.find(d => d.driverId === dId);
                                                    if (driver) assignDriver(booking._id, driver._id, booking.status === 'pending' ? 'pickup' : 'delivery');
                                                }
                                            }}
                                            className="text-[9px] font-black uppercase text-blue-600 hover:text-blue-800 transition"
                                        >
                                            Assign Driver
                                         </button>
                                    </div>
